...
import { CheckCircle2, ClipboardList, DownloadCloud, FileSpreadsheet, Info, AlertCircle, FileText, TrendingUp, Calculator } from 'lucide-react';
import { Card } from '../../components/Card';
import { Button } from '../../components/Button';
import { Table } from '../../components/Table';
import { useAppData, type Engagement, type Purchase, type Client } from '../../store/useAppData';
import { formatCurrency, type ClientInvoice, type VendorInvoice } from './accountingTypes';
import {
...
const AccountingExportPage = () => {
    ...
    return [
      {
        ...
        value: exportPreviewRows.length.toLocaleString('fr-FR'),
        helper: `${clientInvoices.length} factures clients`,
      },
      {
        id: 'total-debit',
        label: 'Total débit',
        value: formatCurrency(totalDebit),
        helper: 'Montant total débité',
      },
      {
        id: 'balance',
        label: balanceCheck.balanced ? 'Équilibre vérifié' : 'Déséquilibre',
        value: balanceCheck.balanced ? '✓' : formatCurrency(Math.abs(balanceCheck.difference)),
        helper: balanceCheck.balanced ? 'Débit = Crédit' : `Différence ${formatCurrency(balanceCheck.difference)}`,
      },
    ];
  }, [exportPreviewRows.length, clientInvoices.length, totalDebit, balanceCheck]);
  ...
  return (
    <div className="dashboard-page space-y-10">
      <header className="dashboard-hero">
        <div className="dashboard-hero__content">
          <div className="dashboard-hero__intro">
            ...
            <h1 className="dashboard-hero__title">Export comptable</h1>
            <p className="dashboard-hero__subtitle">
              Préparez les écritures pour votre cabinet, contrôlez les montants avant envoi et conservez un historique des exports.
            </p>
          </div>
        </div>
      ...
      <CRMFeedback message={message} />

      <section className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {exportKpis.map((kpi, index) => {
            const Icon = [FileText, TrendingUp, Calculator][index] ?? FileText;
            return (
      ...
      <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition-colors md:p-5 dark:border-[var(--border)] dark:bg-[var(--surface)]">
        ...
        <div className="space-y-4">
          {exportPreviewRows.length === 0 ? (
            <CRMEmptyState
              message="Aucune écriture à exporter. Ajoutez des factures clients ou fournisseurs."
            />
          ) : (
            <>
                ...
                {exportPreviewRows.map((row, index) => (
                  ...
                  >
                    <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300">
                      <div className="flex items-center justify-between">
                        <span>Compte</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-100">{row.account}</span>
                      </div>
                      <div className="flex items-center justify-between">
                        <span>Libellé</span>
