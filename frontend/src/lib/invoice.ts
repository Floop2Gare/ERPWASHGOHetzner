...
// Interface pour les prestations multiples dans un devis
...
export interface GenerateQuoteWithMultipleServicesPayload {
  ...
  company: Company & { vatNumber?: string; iban?: string; bic?: string };
  client: Client;
  services: QuoteServiceItem[]; // Tableau de prestations
  vatRate: number;
  vatEnabled: boolean;
  validityNote?: string | null;
}
...
const generateCommercialDocumentPdf = (
  ...
  const headingTitle = mode === 'invoice' 
    ? 'FACTURE' 
    : (vatEnabled ? 'DEVIS' : 'OFFRE DE PRIX');
  ...
  // Pour les offres de prix (vatEnabled = false), on n'affiche pas de total
  // Pour les devis (vatEnabled = true), on affiche le sous-total HT, la TVA et le total TTC
  const totalsData = vatEnabled
    ? [
    ...
    : [
        // Pour les offres de prix, on affiche seulement le détail par prestation, pas de total
        // totalsData reste vide pour les offres de prix
      ];

  // Afficher les totaux seulement s'il y en a (pour les devis)
  if (totalsData.length > 0) {
    ...
    // Pour les offres de prix, on ajoute juste un espace après le tableau
    y += SPACING.xl;
  }
...
// Export de la fonction pour plusieurs prestations
export { 
  generateQuotePdfWithMultipleServices, 
  generateInvoicePdfWithMultipleServices,
  type QuoteServiceItem, 
  type GenerateQuoteWithMultipleServicesPayload,
  type GenerateInvoiceWithMultipleServicesPayload,
  generateQuoteFileName, 
  generateInvoiceFileName 
} from './invoice-multiple-services';
// Interface pour les prestations multiples dans un devis
export interface QuoteServiceItem {
  serviceId: string;
  serviceName: string;
