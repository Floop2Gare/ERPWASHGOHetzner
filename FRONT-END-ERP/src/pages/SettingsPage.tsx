...
const SettingsPage = () => {
  ...
  const isCompanyDetailOpen = detailState?.section === 'companies';
  const isEditingCompanyDetail = isCompanyDetailOpen && detailState?.mode === 'edit';
  const companyModalEyebrow = isEditingCompanyDetail ? "MODIFIER L'ENTREPRISE" : 'CRÉER UNE ENTREPRISE';
  const companyModalTitle = isEditingCompanyDetail
    ? companyForm.name || "Modifier l'entreprise"
    : 'Ajouter une entreprise';
  const companyModalSubtitle = isEditingCompanyDetail
    ? "Actualisez les informations légales, TVA et visuels de l'entreprise."
    : "Renseignez les informations légales, TVA et visuels pour créer une nouvelle entité.";
  const companyModalCtaLabel = isEditingCompanyDetail ? 'Enregistrer les modifications' : "Créer l’entreprise";

  useEffect(() => {
    if (!isCompanyDetailOpen) {
      return;
    }
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="space-y-12">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-colors dark:border-[var(--border)] dark:bg-[var(--surface)]">
                    {sortedCatalogServices.length > 0 ? (
                      <>
                        <div className="hidden lg:block">
                          <table className="w-full text-sm">
                            <thead className="border-b border-slate-200 bg-slate-50 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-300">
                              <tr>
                                ...
                                <th className="px-4 py-3 text-left w-[18%]">
                                  <button
                                    ...
                                    className="flex items-center gap-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500 hover:text-slate-900 dark:text-slate-300 dark:hover:text-slate-100"
                                  >
                                    Catégorie
                                    {catalogServiceSort.key === 'category' && (
                                      <span className="text-[10px]">{catalogServiceSort.direction === 'asc' ? '▲' : '▼'}</span>
                                    )}
                                  </button>
                                </th>
                                <th className="px-4 py-3 text-left w-[18%]">
                            ...
                            <tbody className="divide-y divide-slate-100">
                              {sortedCatalogServices.map((service) => {
                                const activeOptions = service.options.filter((option) => option.active).length;
                                return (
                                  <tr
                                    key={service.id}
                                    ...
                                    className={clsx(
                                      ...
                                      selectedCatalogService?.id === service.id && 'bg-blue-50/60 dark:bg-blue-500/10'
                                    )}
                                  >
                                    <td className="px-4 py-4 align-top">
                                      ...
                                      {service.description ? (
                                        ...
                                        <p className="text-xs text-slate-400">Aucune description</p>
                                      )}
                                    </td>
                                    <td className="px-4 py-4 align-top">
                                      <span className="inline-flex items-center rounded-full bg-purple-600 px-3 py-1.5 text-xs font-bold text-white shadow-md dark:bg-purple-500">
                                        {service.category}
                                      </span>
                                    </td>
                                    <td className="px-4 py-4 align-top">
                                      <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                                        {service.options.length} éléments
                                      </p>
                                      <p className="text-xs text-slate-500 dark:text-slate-400">{activeOptions} actifs</p>
                                    </td>
                                    <td className="px-4 py-4 align-top">
                                      <span
                                        className={clsx(
                                          'inline-flex items-center rounded-full px-3 py-1.5 text-xs font-bold text-white shadow-md',
                                          service.active
                                    ...
                                    <td className="px-4 py-4 align-top">
                                      <div className="flex flex-wrap gap-1.5 text-slate-500 dark:text-slate-300">
                                        <button
                                          type="button"
                                          onClick={(event) => {
                                            event.stopPropagation();
                                          ...
                                          className="rounded-lg p-2 transition hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-slate-800 dark:hover:text-slate-100"
                                          title="Modifier"
                                        >
                                          <IconEdit />
                                        </button>
                                        <button
                                          ...
                                          className="rounded-lg p-2 transition hover:bg-blue-100 hover:text-blue-700 dark:hover:bg-blue-900/20 dark:hover:text-blue-200"
                                          title="Dupliquer"
                                        >
                                          <IconDuplicate />
                                        </button>
                                        <button
                                          ...
                                          className="rounded-lg p-2 transition hover:bg-rose-100 hover:text-rose-600 dark:hover:bg-rose-900/30 dark:hover:text-rose-200"
                                          title="Supprimer"
                                        >
                                          <IconTrash />
                                        </button>
                          ...
                          {sortedCatalogServices.map((service) => {
                            ...
                            return (
                              <div
                                ...
                                onClick={() => setSelectedCatalogServiceId(service.id)}
                                className={clsx(
                                  'rounded-2xl border border-slate-200 p-5 text-sm shadow-sm dark:border-[var(--border)] dark:bg-[var(--surface)]',
                                  selectedCatalogService?.id === service.id && 'border-blue-500 bg-blue-50/40 dark:bg-blue-500/10'
                                )}
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0 flex-1">
                                    <p className="text-base font-semibold text-slate-900 dark:text-slate-100">{service.name}</p>
                                    {service.description ? (
                                      <p className="text-xs text-slate-500 dark:text-slate-400">{service.description}</p>
                                    ) : (
                                      <p className="text-xs text-slate-400">Aucune description</p>
                                  ...
                                  <span
                                    className={clsx(
                                      'rounded-full px-3 py-1.5 text-[11px] font-bold text-white shadow-md',
                                      service.active
                                        ? 'bg-green-600 dark:bg-green-500'
                                        : 'bg-slate-600 dark:bg-slate-500'
                                    )}
                                ...
                                <div className="mt-3 flex flex-wrap gap-2 text-[11px] text-slate-500 dark:text-slate-400">
                                  <span className="rounded-full bg-purple-600 px-3 py-1.5 text-[11px] font-bold text-white shadow-md dark:bg-purple-500">
                                    {service.category}
                                  </span>
                                  <span className="rounded-full bg-slate-100 px-2 py-0.5 font-semibold dark:bg-slate-800">
                                    {service.options.length} prestations
                                  </span>
                                  <span className="rounded-full bg-slate-100 px-2 py-0.5 font-semibold dark:bg-slate-800">
                                    {activeOptions} actives
                                  </span>
                                </div>
                                <div className="mt-4 flex flex-wrap gap-2 border-t border-dashed border-slate-200 pt-3 text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
                                  <button
                                    type="button"
                                    onClick={(event) => {
                                      event.stopPropagation();
                                    ...
                                    className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-slate-50 px-3 py-2 font-semibold text-slate-700 transition hover:bg-slate-100 dark:bg-slate-800 dark:text-slate-200"
                                  >
                                    <IconEdit />
                                    Modifier
                                  </button>
                                  <button
                                    ...
                                    className="flex flex-1 items-center justify-center gap-2 rounded-lg bg-blue-50 px-3 py-2 font-semibold text-blue-700 transition hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-200"
                                  >
                                    <IconDuplicate />
                                    Dupliquer
                                  </button>
                                  <button
                                    ...
                                    className="rounded-lg bg-rose-50 p-2 text-rose-600 transition hover:bg-rose-100 dark:bg-rose-900/20 dark:text-rose-200"
                                    title="Supprimer"
                                  >
                                    <IconTrash />
                                  </button>
                      ...
                      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center dark:border-slate-700 dark:bg-slate-900/40">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400">Créez votre premier service pour commencer.</p>
                      </div>
                    )}
                  </div>
                </Card>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-colors dark:border-[var(--border)] dark:bg-[var(--surface)]">
                        {sortedCatalogItems.length > 0 ? (
                          <>
                              ...
                              <table className="w-full text-sm">
                                    ...
                                    <th className="px-4 py-3 text-left w-[30%]">
                                      ...
                                      >
                                        ...
                                        {catalogItemSort.key === 'label' && (
                                          <span className="text-[10px]">{catalogItemSort.direction === 'asc' ? '▲' : '▼'}</span>
                                        )}
                                      </button>
                                    </th>
                                    <th className="px-4 py-3 text-left w-[18%]">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                  {sortedCatalogItems.map((option) => (
                                    <tr key={option.id} className="group transition hover:bg-slate-50 dark:hover:bg-white/5">
                                      <td className="px-4 py-4 align-top">
                                        <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{option.label}</p>
                                        {option.description ? (
                                          <p className="text-xs text-slate-500 dark:text-slate-400">{option.description}</p>
                                        ) : (
                                          <p className="text-xs text-slate-400">Sans description</p>
                                        )}
                                      </td>
                                      <td className="px-4 py-4 align-top">
                                        <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                            ...
                            <div className="space-y-4 lg:hidden">
                              {sortedCatalogItems.map((option) => (
                                <div key={option.id} className="rounded-2xl border border-slate-200 p-5 text-sm shadow-sm dark:border-[var(--border)] dark:bg-[var(--surface)]">
                                  <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0 flex-1">
                          ...
                          <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-12 text-center dark:border-slate-700 dark:bg-slate-900/40">
                            <h3 className="mb-2 text-lg font-semibold text-slate-800 dark:text-slate-100">Aucune prestation</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400">
                              Ajoutez votre première prestation pour {selectedCatalogService.name}.
                            </p>
                          </div>
                        )}
                      </div>
        ...
        {detailState && detailState.section !== 'companies' && (
          <div
      ...
      {showProfileModal &&
        ...
        {isCompanyDetailOpen &&
          ...
          createPortal(
            <div
              className="fixed inset-0 z-[130] flex items-center justify-center bg-slate-950/40 px-4 py-6 backdrop-blur-md"
              role="dialog"
              aria-modal="true"
              aria-labelledby="company-detail-title"
              onClick={() => closeDetail('companies')}
            >
            <div
              className="relative w-full max-w-6xl max-h-[95vh] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl ring-1 ring-slate-900/10 transition dark:border-slate-700 dark:bg-slate-900"
                onClick={(event) => event.stopPropagation()}
              >
                <form
                  ref={companyDetailFormRef}
                  onSubmit={handleCompanySubmit}
                className="flex h-full flex-col gap-6 bg-white p-6 text-slate-900 dark:bg-slate-900 dark:text-slate-100 md:p-9 max-h-[95vh] overflow-y-auto"
                >
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div className="space-y-1">
                      ...
                      <h2 id="company-detail-title" className="text-xl font-semibold text-slate-900 dark:text-slate-100">
                        {companyModalTitle}
                      </h2>
                      <p className="max-w-2xl text-xs text-slate-500 dark:text-slate-400">{companyModalSubtitle}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => closeDetail('companies')}
                      className="ml-auto flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-100 hover:text-slate-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/40 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                      aria-label="Fermer"
                    >
                      <X className="h-4 w-4" />
                    </button>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <label className={fieldLabelClass}>
                        ...
                        <input
                          ...
                          value={companyForm.country}
                          onChange={handleCompanyFormChange}
                          className={`${inputClass} md:max-w-[200px]`}
                          required
                        />
                      </label>
                    </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                      <label className="flex items-center gap-2 rounded-soft border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                        <input
                          type="checkbox"
                          name="vatEnabled"
                          checked={companyForm.vatEnabled}
                      ...
                      <label className="flex flex-col gap-2 rounded-soft border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                        ...
                        <div className="flex flex-1 flex-col gap-2 text-xs text-slate-500">
                          ...
                          <p className="text-[11px] text-slate-400">
                            Les logos importés sont stockés en local pour accélérer vos exports.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3">
                        <label className={fieldLabelClass}>
                          <span>Titre du bandeau</span>
                          <input
                            name="documentHeaderTitle"
                            value={companyForm.documentHeaderTitle}
                            onChange={handleCompanyFormChange}
                            className={inputClass}
                            placeholder="Wash&Go Services"
                          />
                        </label>
                        <label className={fieldLabelClass}>
                          <span>Sous-titre</span>
                          <input
                            ...
                            onChange={handleCompanyFormChange}
                            className={inputClass}
                            placeholder="Spécialiste du detailing automobile"
                          />
                        </label>
                        <label className={fieldLabelClass}>
                          ...
                          <textarea
                            ...
                            rows={3}
                            className={textareaClass}
                            placeholder="Informations affichées sur vos devis/factures"
                          />
                        </label>
                      </div>
                    </div>
                  ...
                  <div className="flex flex-wrap justify-end gap-2 border-t border-slate-200 pt-3 dark:border-slate-800">
                    <Button type="button" variant="ghost" onClick={() => closeDetail('companies')}>
                      Annuler
                    </Button>
                    <Button type="submit">{companyModalCtaLabel}</Button>
                  </div>
                </form>
              </div>
            </div>,
