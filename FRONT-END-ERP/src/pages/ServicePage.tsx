...
const ServicePage = () => {
  ...
  const handleCreateService = async (options?: { sendAsQuote?: boolean }) => {
    ...
    const engagement = addEngagement({
      ...
      status: creationDraft.kind === 'devis' ? 'brouillon' : creationDraft.status,
      ...
      invoiceVatEnabled: null,
      quoteNumber: null,
      quoteStatus: creationDraft.kind === 'devis' ? 'brouillon' : null,
      // Planning
  ...
  const handleConvertQuoteToService = async (engagement: Engagement) => {
    if (engagement.kind !== 'devis') {
      setFeedback('Seuls les devis peuvent être convertis en service.');
      return;
    }
    ...
    try {
      const updated = updateEngagement(engagement.id, {
        kind: 'service',
        status: engagement.status === 'brouillon' ? 'planifié' : engagement.status,
        quoteStatus: null, // On retire le statut de devis
      });

      if (!updated) {
        setFeedback('Erreur lors de la conversion du devis en service.');
        return;
      }

      setFeedback('Devis converti en service avec succès.');
      setSelectedEngagementId(updated.id);
      setEditDraft(buildDraftFromEngagement(updated));
      scrollToList();
    } catch (error) {
      console.error('[Wash&Go] Erreur lors de la conversion du devis en service', error);
      setFeedback('Une erreur est survenue lors de la conversion. Veuillez réessayer.');
    }
  };
...
const handleEditSubmit = (event: FormEvent<HTMLFormElement>) => {
  ...
  const renderDesktopActions = (engagement: Engagement) => (
    <div className="flex items-center justify-end gap-2 opacity-100 transition group-hover:translate-x-[1px] group-hover:opacity-100">
      ...
      {hasPermission('service.create') && engagement.kind === 'devis' && (
        <button
          type="button"
          onClick={(event) => {
            event.stopPropagation();
            void handleConvertQuoteToService(engagement);
          }}
  ...
  const renderMobileActions = (engagement: Engagement) => {
    const actions = [
      ...
      hasPermission('service.create') && engagement.kind === 'devis' && {
        key: 'convert',
        label: 'Convertir en service',
        onClick: () => {
          void handleConvertQuoteToService(engagement);
        },
        className: 'bg-green-50 text-green-700 hover:bg-green-100 dark:bg-green-900/30 dark:text-green-200',
        icon: <IconConvert />,
      },
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div className="space-y-1">
                      <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-blue-500">
                        {creationDraft.kind === 'devis' ? 'NOUVEAU DEVIS' : 'NOUVEAU SERVICE'}
                      </span>
                  ...
                  {/* Sélection du type : Service ou Devis */}
                  <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <label className="mb-2 block text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400">
                      Type de document *
                    </label>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        type="button"
                        onClick={() => setCreationDraft((draft) => ({ ...draft, kind: 'service', status: draft.status === 'brouillon' ? 'planifié' : draft.status }))}
                        className={clsx(
                          'rounded-lg border px-4 py-3 text-sm font-medium transition-colors',
                          creationDraft.kind === 'service'
                      ...
                      <button
                        type="button"
                        onClick={() => setCreationDraft((draft) => ({ ...draft, kind: 'devis', status: 'brouillon' }))}
                        className={clsx(
                          'rounded-lg border px-4 py-3 text-sm font-medium transition-colors',
                          creationDraft.kind === 'devis'
                            ? 'border-purple-600 bg-purple-600 text-white dark:bg-purple-500'
                            : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300'
                        )}
                      >
                        Devis
                    ...
                    <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                      {creationDraft.kind === 'devis'
                        ? 'Un devis sera créé en brouillon et pourra être envoyé au client.'
                        : 'Un service sera créé avec le statut sélectionné.'}
                    </p>
                  </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            disabled={creationDraft.kind === 'devis'}
                            className={clsx(
                              ...
                              creationDraft.kind === 'devis' && 'cursor-not-allowed opacity-60 bg-slate-100 dark:bg-slate-700'
                            )}
                            required
                          >
                            <option value="brouillon">Brouillon</option>
                  ...
                  <div className="mt-2 flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 pt-3 dark:border-slate-800">
                    ...
                    <button
                      type="submit"
                      className={clsx(
                        'inline-flex items-center justify-center gap-2 rounded-lg px-4 py-1.5 text-xs font-semibold text-white shadow-lg transition focus:outline-none focus-visible:ring-2',
                        creationDraft.kind === 'devis'
                          ? 'bg-purple-600 shadow-purple-600/25 hover:bg-purple-700 dark:bg-purple-500 dark:hover:bg-purple-600 focus-visible:ring-purple-500/60'
                          : 'bg-blue-600 shadow-blue-600/25 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 focus-visible:ring-blue-500/60'
                      )}
                    >
                      <Plus className="h-3 w-3" />
                      {creationDraft.kind === 'devis' ? 'Créer le devis' : 'Créer le service'}
                    </button>
                  </div>
                </form>
              </div>
