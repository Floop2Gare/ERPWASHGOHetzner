  ...
  return (
    <div className="purchases-page space-y-10">
      ...
      {activePanel &&
        ...
        createPortal(
          <div
            className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/40 px-4 py-6 backdrop-blur-md"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div
              className="relative w-full max-w-3xl max-h-[90vh] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl ring-1 ring-slate-900/10 transition dark:border-slate-700 dark:bg-slate-900"
              onClick={(event) => event.stopPropagation()}
            >
              <form
                ref={purchaseModalFormRef}
                onSubmit={handleSubmit}
                className="flex h-full flex-col gap-5 bg-white p-4 text-slate-900 dark:bg-slate-900 dark:text-slate-100 md:p-6 max-h-[90vh] overflow-y-auto"
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div className="space-y-1">
                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-blue-500">
                      {purchaseModalEyebrow}
                    </span>
                    <h2 id="purchase-modal-title" className="text-xl font-semibold text-slate-900 dark:text-slate-100">
                      {purchaseModalTitle}
                    </h2>
                    <p className="max-w-xl text-xs text-slate-500 dark:text-slate-400">{purchaseModalSubtitle}</p>
                  </div>
                  <button
                    type="button"
                    onClick={closeForm}
